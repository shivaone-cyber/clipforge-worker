...
const STRIP_H = 120;
const PRODUCTION_PROTECTED_TOP = 196;
const PRODUCTION_STRIP_TOP = 1096;
const PRODUCTION_STRIP_BOTTOM = 1648;
const PRODUCTION_OUTER_PADDING = 272;
...
async function main() {
  ...
  // Exact production regression. Story analysis was calculated from the
  // protected block at y=196..1648, then an AI plan supplied a stale/narrower
  // content rectangle. The renderer must use the absolute protected source
  // coordinates, exclude only the 272px outer padding, and never refuse.
  const productionInput = join(OUT_DIR, "production-geometry-src.mp4");
  await run("ffmpeg", ["-y", "-hide_banner", "-loglevel", "error",
    "-f", "lavfi", "-i", "testsrc2=size=1080x1920:rate=30:duration=6",
    "-f", "lavfi", "-i", "sine=frequency=550:duration=6",
    "-vf", `drawbox=x=0:y=0:w=1080:h=${PRODUCTION_PROTECTED_TOP}:color=black@1.0:t=fill,drawbox=x=0:y=${PRODUCTION_STRIP_TOP}:w=1080:h=${PRODUCTION_STRIP_BOTTOM - PRODUCTION_STRIP_TOP}:color=magenta@1.0:t=fill,drawbox=x=0:y=${PRODUCTION_STRIP_BOTTOM}:w=1080:h=${PRODUCTION_OUTER_PADDING}:color=black@1.0:t=fill`,
    "-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p", "-c:a", "aac", "-shortest", productionInput]);
  const productionMeta = await probe(productionInput);
  productionMeta.content_rect = { x: 0, y: 300, w: 1080, h: 1200 };
  productionMeta.story_regions = {
    sourceTop: PRODUCTION_PROTECTED_TOP,
    sourceBottom: 1920,
    protectedStoryTop: PRODUCTION_PROTECTED_TOP,
    upperHookRegion: { top: 196, bottom: 422 },
    mainContentRegion: { top: 196, bottom: PRODUCTION_STRIP_TOP },
    emptyLowerRegion: { top: PRODUCTION_STRIP_TOP, bottom: PRODUCTION_STRIP_TOP },
    kickStripRegion: { top: PRODUCTION_STRIP_TOP, bottom: PRODUCTION_STRIP_BOTTOM },
    originalSourceForegroundHeight: 1452,
    expendableLowerHeight: 0,
    outerPaddingRegion: { top: PRODUCTION_STRIP_BOTTOM, bottom: 1920 },
    outerPaddingHeight: PRODUCTION_OUTER_PADDING,
    outerPaddingRemovedPixels: PRODUCTION_OUTER_PADDING,
    actualRemovedPixels: 0,
    removalMode: "NONE",
  };
  const productionSourceFrame = await frameRGB(productionInput, 2, 1080, 1920);
  const productionSourceStrip = stripRows(productionSourceFrame);
  const productionOutput = join(OUT_DIR, "production-geometry-smartcrop-blur.mp4");
  let productionBuilt = null;
  let productionError = null;
  try {
    productionBuilt = await render({
  ...
  check("production geometry render succeeds", productionError === null, productionError ? `${productionError.reason ?? ""} ${productionError.message}` : "");
  check("production geometry has no protected-strip refusal", !String(productionError?.message ?? "").includes("KICK strip/protected story block"));
  if (productionBuilt) {
    const productionOutMeta = await probe(productionOutput);
    const productionOutputFrame = await frameRGB(productionOutput, 2, productionOutMeta.width, productionOutMeta.height);
    const productionOutputStrip = stripRows(productionOutputFrame);
    const productionScale = productionBuilt.vh / productionBuilt.crop.h;
    const productionExpectedTop = Math.round(productionBuilt.vy + (PRODUCTION_STRIP_TOP - productionBuilt.crop.y) * productionScale);
    const productionExpectedHeight = Math.round((PRODUCTION_STRIP_BOTTOM - PRODUCTION_STRIP_TOP) * productionScale);
    check("production protected foreground is not cropped", productionBuilt.crop.y === PRODUCTION_PROTECTED_TOP && productionBuilt.crop.h === 1452 && productionBuilt.cropTop === 0 && productionBuilt.cropBottom === 0, JSON.stringify(productionBuilt.crop));
    check("production protected-safe fallback recorded", productionBuilt.framingFallback === "PROTECTED_SAFE_FIT", `${productionBuilt.framingFallback} fit=${productionBuilt.foregroundFit}`);
    check("production KICK strip protected", productionBuilt.kickStripProtected === true);
    check("production KICK strip source geometry detected", productionSourceStrip.top === PRODUCTION_STRIP_TOP && productionSourceStrip.height === PRODUCTION_STRIP_BOTTOM - PRODUCTION_STRIP_TOP, JSON.stringify(productionSourceStrip));
    check("production KICK strip output height intact", Math.abs(productionOutputStrip.height - productionExpectedHeight) <= 4, `actual=${productionOutputStrip.height} expected=${productionExpectedHeight}`);
    check("production KICK strip output position intact", Math.abs(productionOutputStrip.top - productionExpectedTop) <= 4, `actual=${productionOutputStrip.top} expected=${productionExpectedTop}`);
    check("production analysis keeps outer padding separate", productionMeta.story_regions.expendableLowerHeight === 0 && productionMeta.story_regions.outerPaddingHeight === 272 && productionMeta.story_regions.outerPaddingRemovedPixels === 272 && productionMeta.story_regions.actualRemovedPixels === 0);
    check("production balanced-margin trim unchanged", productionBuilt.trimScope === "OUTER_BACKGROUND_ONLY" && productionBuilt.actualRemovedPixels === 0 && productionBuilt.trimSkippedReason === "BALANCED_ALREADY", JSON.stringify({ actual: productionBuilt.actualRemovedPixels, reason: productionBuilt.trimSkippedReason }));
    check("production final canvas follows unchanged trim", productionBuilt.finalCanvasHeight === 1920 - productionBuilt.actualRemovedPixels, `${productionBuilt.finalCanvasHeight}`);
  }
  ...
  console.log(JSON.stringify({
    ...
    productionRegression: productionBuilt ? {
      crop: productionBuilt.crop,
      framingFallback: productionBuilt.framingFallback,
      foregroundFit: productionBuilt.foregroundFit,
      kickStripProtected: productionBuilt.kickStripProtected,
      trim: { actualPixels: productionBuilt.actualRemovedPixels, skipped: productionBuilt.trimSkippedReason },
    } : null,
  }, null, 2));
