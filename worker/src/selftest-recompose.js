...
const CANVAS_W = 1080;
// The approved balanced-margin trim intentionally shortens the final canvas, so
// the harness must never assume 1920 — it reads the real output height instead.
const NOMINAL_CANVAS_H = 1920;

async function videoHeight(file) {
  const { stdout } = await run("ffprobe", ["-v", "error", "-select_streams", "v:0", "-show_entries", "stream=height", "-of", "csv=p=0", file]);
...
function measure(buf, tolerance = 14, CANVAS_H = NOMINAL_CANVAS_H) {
  const px = (x, y) => {
...
async function renderCase(label, input, config, t = 1) {
  ...
  const actualHeight = await videoHeight(output);
  const geo = measure(buf, config.background?.style === "black" ? 24 : 14, actualHeight);
  await writeFile(join(OUT_DIR, `${label}.log`), `${logs.join("\n")}\n${JSON.stringify({ built, geo, actualHeight }, null, 2)}`);
  return { built, geo, buf, output, actualHeight };
}
...
async function main() {
  ...
  check("T1 complete vertical coverage of final canvas", bad.built.backgroundTopHeight + bad.built.vh + bad.built.backgroundBottomHeight === bad.actualHeight, `top=${bad.built.backgroundTopHeight}px foreground=${bad.built.vh}px bottom=${bad.built.backgroundBottomHeight}px canvas=${bad.actualHeight}px`);
  check("T1 output height matches approved trim decision", bad.actualHeight === bad.built.finalCanvasHeight && bad.actualHeight === NOMINAL_CANVAS_H - bad.built.actualRemovedPixels, `actual=${bad.actualHeight} reported=${bad.built.finalCanvasHeight} trim=${bad.built.actualRemovedPixels}`);
  check("T1 trim is outer background only", bad.built.trimScope === "OUTER_BACKGROUND_ONLY" && bad.built.foregroundChanged === false && bad.built.kickStripChanged === false, `scope=${bad.built.trimScope} fgChanged=${bad.built.foregroundChanged} stripChanged=${bad.built.kickStripChanged}`);
  check("T1 trim never exceeds lower background", bad.built.actualRemovedPixels <= bad.built.lowerBackgroundHeightBefore && bad.built.lowerBackgroundHeightAfter === bad.built.lowerBackgroundHeightBefore - bad.built.actualRemovedPixels, `before=${bad.built.lowerBackgroundHeightBefore} after=${bad.built.lowerBackgroundHeightAfter} trim=${bad.built.actualRemovedPixels}`);
  check("T1 protected foreground untouched by trim", bad.built.vy >= 0 && bad.built.vy + bad.built.vh <= bad.actualHeight && bad.built.cropAmount.width <= 1, `vy=${bad.built.vy} vh=${bad.built.vh} canvas=${bad.actualHeight}`);
  ...
  for (const [name, other] of [["black", black], ["blur", blur]]) {
    ...
    check(`T2 ${name} canvas height identical`, other.actualHeight === bad.actualHeight, `${other.actualHeight} vs ${bad.actualHeight}`);
  ...
  check("T3 full frame", orig.built.sourceMode === "FULL_FRAME_9_16" && orig.built.vw === CANVAS_W && orig.built.vh === orig.actualHeight && orig.built.vy === 0, `mode=${orig.built.sourceMode} ${orig.built.vw}x${orig.built.vh}@${orig.built.vx},${orig.built.vy}`);
  ...
  if (target) {
    ...
    check("T5 target coverage", tg.built.backgroundTopHeight + tg.built.vh + tg.built.backgroundBottomHeight === tg.actualHeight, `top=${tg.built.backgroundTopHeight} foreground=${tg.built.vh} bottom=${tg.built.backgroundBottomHeight} canvas=${tg.actualHeight}`);
    check("T5 target canvas matches trim decision", tg.actualHeight === NOMINAL_CANVAS_H - tg.built.actualRemovedPixels, `canvas=${tg.actualHeight} trim=${tg.built.actualRemovedPixels}`);
  }
