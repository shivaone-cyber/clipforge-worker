...
export function buildFilterGraph(config, meta, resolved) {
  ...
  let content = sourceMode === "FULL_FRAME_9_16" ? decision.rect : detectedRect;

  // Story-region analysis is deterministic and uses absolute source pixels.
  // An AI content rectangle can arrive later and must never narrow the source
  // window past that already-protected full-width foreground. Normalize the
  // Kick Story source window to the complete protected block before any Smart
  // Crop math. Outer padding below the KICK strip remains outside this source
  // window and is not mistaken for expendable foreground.
  if (sourceMode === "KICK_STORY_RECOMPOSE") {
    const protectedTop = Number(meta.story_regions?.protectedStoryTop);
    const protectedBottom = Number(meta.story_regions?.kickStripRegion?.bottom);
    const validProtectedSpan = Number.isFinite(protectedTop)
      && Number.isFinite(protectedBottom)
      && protectedTop >= 0
      && protectedBottom > protectedTop
      && protectedBottom <= meta.height;
    if (validProtectedSpan) {
      content = {
        x: 0,
        y: protectedTop,
        w: meta.width,
        h: protectedBottom - protectedTop,
      };
    }
  }
...
function buildKickStoryRecomposeGraph({ config, meta, resolved, canvasW, canvasH, content, gradeFilter, detected }) {
  ...
  let topH = clampInt(config.video?.headlineHeight ?? canvasH * 0.245, 240, Math.round(canvasH * 0.32), Math.round(canvasH * 0.245));
  ...
  // Protected-safe framing fallback. Smart Crop (and any other framing) may
  // only remove confirmed non-protected source rows. When no candidate window
  // can hold the complete protected block — upper hook, footage, creator
  // overlay and the KICK strip — the crop is widened back to the full
  // protected span instead of failing the job. Nothing inside the protected
  // span is ever cropped, masked, moved or recoloured.
  const declaredProtectedBottomPx = protectedRegions.length
    ? Math.max(...protectedRegions.map((r) => (Number(r.y) + Number(r.height)) * content.h))
    : 0;
  const kickTopAbsolute = Number(regions?.kickStripRegion?.top ?? content.y);
  const kickBottomAbsolute = Number(regions?.kickStripRegion?.bottom ?? content.y + content.h);
  const protectedTopRelative = Math.max(0, Number(regions?.protectedStoryTop ?? content.y) - content.y);
  const kickBottomRelative = kickBottomAbsolute - content.y;
  const protectedBottomPx = Math.min(
    content.h,
    Math.ceil(Math.max(declaredProtectedBottomPx, kickBottomRelative, 2)),
  );
  let framingFallback = null;
  if (cy > 0 || cy + ch < protectedBottomPx) {
    const evenContentH = content.h - (content.h % 2);
    const wanted = protectedBottomPx + (protectedBottomPx % 2);
    cy = 0;
    ch = Math.max(2, Math.min(evenContentH, wanted));
    framingFallback = "PROTECTED_SAFE_FIT";
  }
  ...
  // The KICK strip sits at the bottom edge of the embedded story block.
  const kickStripProtected = protectedTopRelative + content.y >= crop.y
    && kickTopAbsolute >= crop.y
    && kickBottomAbsolute <= crop.y + crop.h;
  // Scale proportionally to exactly canvas width; height follows the crop.
  // When the complete protected block cannot fit at full canvas width, the
  // headline region is tightened first and only then is the whole foreground
  // scaled down proportionally (never cropped) and centred, with the selected
  // background filling the remaining outer area.
  let vw = canvasW;
  let vh = roundEven((crop.h * canvasW) / cw);
  let vx = 0;
  let foregroundFit = "FULL_WIDTH";
  if (topH + vh > canvasH) {
    topH = Math.max(Math.round(canvasH * 0.18), Math.min(topH, canvasH - vh));
    if (topH + vh > canvasH) {
      vh = floorEven(canvasH - topH);
      vw = Math.min(canvasW, roundEven((cw * vh) / crop.h));
      vx = floorEven((canvasW - vw) / 2);
      foregroundFit = "SCALED_TO_FIT";
      framingFallback = framingFallback ?? "PROTECTED_SAFE_FIT";
    }
  }
...
export async function render({ input, output, config, meta, resolved, onLog }) {
  ...
  // Full width stays mandatory unless protected-safe framing had to scale the
  // untouched foreground down so the complete protected block (upper hook,
  // footage, creator overlay, KICK strip) survives without any crop.
  const protectedSafeFit = built.foregroundFit === "SCALED_TO_FIT" && built.kickStripProtected === true;
  if (requestedMode === "kick_story" && !protectedSafeFit && (built.vx !== 0 || built.vw !== built.canvasW)) {
    throw Object.assign(
      new Error(`kick_story foreground not full width (vx=${built.vx} vw=${built.vw})`),
      { reason: "geometry", retryable: false },
    );
  }
  if (requestedMode === "kick_story" && protectedSafeFit && Math.abs(built.vx - (built.canvasW - built.vx - built.vw)) > 2) {
    throw Object.assign(
      new Error(`protected-safe foreground not centred (vx=${built.vx} vw=${built.vw})`),
      { reason: "geometry", retryable: false },
    );
  }
  const args = [
